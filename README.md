...
当前项目支持 Vercel 作品集预览：本地后端可用时使用真实数据；线上没有 `/api` 后端时，会自动进入只读演示数据，保证面试官打开链接能看到产品形态。

真实跨设备使用还需要云端 API、数据库和用户登录，不能只依赖 Vercel 静态前端。具体步骤见 [DEPLOYMENT.md](DEPLOYMENT.md)。
